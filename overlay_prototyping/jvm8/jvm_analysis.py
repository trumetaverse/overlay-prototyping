...
parse_cls_line = lambda line: (int(line.split()[1], 16), line.split('(name=')[1].split(',')[0])
...
def create_bci_ranges(methods):
    ...
    for m in methods:
        ...
        bci_range['values'] = set([i for i in range(bci_range['start'], bci_range['end'])])
...
def build_found_value_summarys(value_range_locs):
    ...
    for baddr in skeys:
        logs.append(
            "Range: 0x%08x # of addrs %5d, # of values %5d" % (baddr, found_addrs_locs[baddr], found_val_locs[baddr]))
    return logs
...
def impl_par_scan_page_for_dword_values(arg_items):
    ...
    if isinstance(values, list):
        values = set(values)
    elif isinstance(values, int) or isinstance(values, int):
        x = set()
        x.add(values)
        values = x
...
class JVMAnalysis(object):
    ...
    def __init__(self, mem_ranges, os_type="linux", libjvm={},
                 ...
                 start_symt_bf=None, end_symt_bf=None, start_sys_dict_bf=None, end_sys_dict_bf=None, start_strt_bf=None,
                 end_strt_bf=None):
        ...
        self._init_params = {'mem_ranges': mem_ranges, 'os_type': os_type,
                             ...
                             'redis_strings_con': redis_strings_con, 'redis_ptrs_con': redis_ptrs_con,
                             'word_sz': word_sz,
    ...
    def gen_page_tables(self):
        ...
        for r in self.ranges.values():
            ...
            for p in range(addr, end, self.page_size):
    ...
    def calc_hardcoded_shared_dictionary(self):
        libjvm_base = self.get_libjvm_base()
        addr = libjvm_base + self.get_version_addr(
            self.get_os() + ':' + SHAREDDICTIONARY + ':' + self.get_java_version())
        # self.log ("Looking for symbol table @ 0x%08x"%addr)
        saddr = self.read_addr(addr)
        # self.log ("Addr @ x%08x = 0x%08x"%(addr, saddr))
        return saddr
    ...
    def lookup_known_klass_by_name(self, name):
        if type(name) == int or type(name) == int:
    ...
    def get_oop_only(self, addr):
        return self.lookup_known_oop_only(addr)
    ...
    def get_string_addresses(self, strings):
        ...
        for i in strings:
            ...
            if len(addrs) > 0:
                string_addresses[i] = [int(j) for j in addrs]
    ...
    def scan_page_for_dword_values(self, r, values, start=None, end=None, omit_ranges=[]):
        ...
        all_vals = found_vals.values()
        self.log(
            "Completed scanning chunk: 0x%08x.  Found %d values, %d unique" % (baddr, len(all_vals), sum(all_vals)))
        return value_addr_locs, value_range_locs

    def scan_pages_for_dword_values(self, values, start=None, end=None, omit_ranges=[], in_parallel=False,
                                    num_procs=10):
        if isinstance(values, list):
            values = set(values)
        elif isinstance(values, int) or isinstance(values, int):
        ...
        if not in_parallel:
            for r in _ranges:
                _value_addr_locs, _value_range_locs = self.scan_page_for_dword_values(r, values, start, end,
                                                                                      omit_ranges)
        ...
        for i in q:
            ...
            try:
                ...
                if not symt is None and len(symt.get_bucket_values()) > self.min_symbols:
                    symts.append(symt)
                    self.log(
                        "Found symbol table cand. at 0x%08x (entrys=%d)" % (symt.addr, len(symt.get_bucket_values())))
            except:
                # import traceback
                # traceback.print_exc()
                pass
