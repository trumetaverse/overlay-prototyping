...
class Oop(BaseOverlay):
    ...
    def accumulate_values(self, to_lookup={}, bread_crumbs={}, found_values={}, set_bread_crumbs=False):
        ...
        if not self.addr in bread_crumbs:
            bread_crumbs[self.addr] = {'is_array': self.is_array_oop(), 'is_prim': self.is_prim(), 'value': {},
                                       'addr': self.addr, 'ref_addrs': set()}
            if set_bread_crumbs:
        ...
        for klass_name in klasses:
            ...
            for field_name in klass_fld_values[klass_name]:
                ...
                # elif oop.addr in found_values:
                    ...
                    res[res_key] = oop.update_values(bread_crumbs=bread_crumbs, found_values=found_values,
                                                     updated=updated)
    ...
    def build_python_value(self, bread_crumbs={}, lookup={}, set_bread_crumbs=False, to_lookup={},
                           recurse_into_oops=True, found_values={}, updated={}):
        ...
        if not self.addr in found_values:
            self.accumulate_values(bread_crumbs=bread_crumbs, to_lookup=to_lookup, found_values=found_values,
                                   set_bread_crumbs=set_bread_crumbs)
    ...
    def python_value(self, klass_name=None, field_name=None, bread_crumbs={}, lookup={}, set_bread_crumbs=False):
        ...
        if klass_name and field_name and \
            ...
            if not oop.addr in lookup:
                ...
                if not oop.is_prim() and not oop.is_array_oop():
                    ...
                    print
                    "updating value: %s len=" % (res_key, len(v))
                else:
            ...
            for field, oop in klass_field_values.items():
                ...
                if not oop.addr in lookup:
                    ...
                    if not oop.is_prim() and not oop.is_array_oop() and self.is_python_native(v):
                        print
                        "[XXXX] OOP is not primitive, but the Python Value is native: %s" % res_key
        ...
        for klass_name in klasses:
            result = self.python_value(klass_name=klass_name, bread_crumbs=bread_crumbs, lookup=lookup,
                                       set_bread_crumbs=set_bread_crumbs)
            res.update(result)
...
class ObjArrayKlassOop(BaseOverlay):
    ...
    def accumulate_values(self, to_lookup={}, bread_crumbs={}, found_values={}, set_bread_crumbs=False):
        ...
        klass_fld_values = getattr(self, 'oop_field_values_by_name', None)
        if not self.addr in bread_crumbs:
            bread_crumbs[self.addr] = {'is_array': True, 'is_prim': (prim in PRIMITIVES_OOPS), 'value': res,
                                       'addr': self.addr, 'ref_addrs': set()}
            if set_bread_crumbs:
                setattr(self, 'oop_dict', bread_crumbs)
...
class ArrayKlassOop(BaseOverlay):
    ...
    def update_fields(self, force_update=False):
        ...
        # read in all the oops that are after
        # header
        jva = self.jvm_analysis
        sz = self.size32 if self.jvm_analysis.is_32bit else \
            self.size64
        ...
        while pos < length:
            o_addr = base + pos * incr
            # print "reading address of 0x%08x"%(o_addr)
            naddr = jva.deref32(o_addr) if self.jvm_analysis.is_32bit else \
                jva.deref64(o_addr)
            oop_addrs.append(o_addr)
            if naddr == 0:
                oop_vals.append(None)
